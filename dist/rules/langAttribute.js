...
exports.langAttributeRule = langAttributeRule;
const traverse_1 = __importDefault(require("@babel/traverse"));
const t = __importStar(require("@babel/types"));
function langAttributeRule(parsed) {
    const issues = [];
    (0, traverse_1.default)(parsed.ast, {
        JSXOpeningElement(path) {
            const name = path.node.name;
            if (!t.isJSXIdentifier(name) || name.name !== 'html')
                return;
            const attrs = path.node.attributes;
            const langAttr = attrs.find((a) => t.isJSXAttribute(a) && t.isJSXIdentifier(a.name) && a.name.name === 'lang');
            if (!langAttr) {
                issues.push({
                    rule: 'lang-attribute',
                    severity: 'error',
